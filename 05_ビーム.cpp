// スコアが下がるしそんなに左上に集まらないのでやめたほうがいい
...
int decide_action_montecarlo(State &state, TimeKeeper &tk) {
  ...
  while (!tk.isTimeOver()) {
    for (int action : legal_actions) {
      int num = 1;
      if (action == UP || action == LEFT) num = 3;
      rep(i, num) {
...
string decide_action_montecarlo_multi(State &state, TimeKeeper &tk,
                                      int multi_turn = 1) {
  ll best_score = 0;
  string best_action = "";
  int loop_count = 0;

  vector<pair<string, State>> legal_actions;
  ...
  auto dfs = [&](auto rec, string action = "", int depth = 0) -> void {
    if (depth == multi_turn) {
      ...
      if (s != "") {
        legal_actions.emplace_back(s, new_state);
      }
      return;
    }
  ...
  while (!tk.isTimeOver()) {
    for (auto [action, new_state] : legal_actions) {
...
int decide_action_beam_search(State &state, TimeKeeper &tk,
  ...
  for (;; t++) {
    ...
    if (best_state.get_legal_actions().empty()) {
      break;
    }
  }
  cerr << "t: " << t << endl;
...
void local_play() {
  ...
  while (1) {
    ...
    // if (turn == 500) depth++;
    // if (turn == 550) depth++;
    // string actions = decide_action_montecarlo_multi(state, tk, depth);
    // if (actions == "") break;
    // for (char c : actions) {
    ...
    int action = decide_action_beam_search(state, tk, 1000);
    if (action == -1) break;
    cerr << action << endl;
    state.advance(action);
    state.debug_board();
...
int main() {
  ...
  state.build();
  {
    ...
    // cout << actions << endl;
    // int action = decide_action_montecarlo(state, tk);
    int action = decide_action_beam_search(state, tk);
    state.advance(action);
